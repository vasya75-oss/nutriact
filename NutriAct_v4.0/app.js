...
function renderApiSetup() {
    ...
    content.innerHTML = `
        <div class="card" style="max-width: 500px; margin: 2rem auto;">
            ...
            <input type="text" id="setupSupabaseUrl" placeholder="https://xxxxx.supabase.co" style="width:100%"><br><br>
            <label>Supabase Anon Key:</label>
            <input type="text" id="setupSupabaseKey" placeholder="eyJ... или sb_publishable_..." style="width:100%"><br><br>
    ...
    document.getElementById('saveSetupBtn').addEventListener('click', async () => {
        ...
        if (!url || !key) {
            showMessage('setupMessage', 'Заполните оба поля', 'error');
            return;
        }
...
async function renderDashboard(container) {
    ...
    const burnedCalories = todayActivity.reduce((sum, e) => sum + (e.calories_burned || 0), 0);
    const waterMl = todayActivity.reduce((sum, e) => sum + (e.water_ml || 0), 0);
        ...
        <div class="page">
            ...
            <div class="grid-2">
                <div class="card">
                    ...
                    ${todayFood.map(f => `<div class="food-item">${f.name} — ${f.calories} ккал (${f.weight}г)</div>`).join('') || '<div>Нет записей</div>'}
                </div>
                <div class="card">
                    <h3>🏃 Активность</h3>
                    ${todayActivity.filter(a => a.calories_burned).map(a => `<div class="activity-item">${a.name} — ${a.calories_burned} ккал</div>`).join('') || '<div>Нет активности</div>'}
                    <h3>💧 Вода</h3>
...
async function renderDiary(container) {
    ...
    for (let date of allDates) {
        ...
        const totalFood = foods.reduce((s, e) => s + (e.calories || 0), 0);
        ...
        html += `
            <div class="card">
                ...
                <details>
                    ...
                    ${foods.map(f => `<div>${f.name} — ${f.calories} ккал (${f.weight}г)</div>`).join('')}
                    ${activities.filter(a => a.calories_burned).map(a => `<div>${a.name} — ${a.calories_burned} ккал</div>`).join('')}
                    ${activities.filter(a => a.water_ml).map(a => `<div>💧 ${a.water_ml} мл</div>`).join('')}
                </details>
            </div>
        `;
    }
